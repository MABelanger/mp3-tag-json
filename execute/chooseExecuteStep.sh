...
STEP1_MP3_PATH='../mp3'
STEP1_TMP_ALL_FILES_PATH="../tmp/step1_all.list"
STEP1_FILE_FILTER=""
STEP2_TMP_FILES_KEEP_PATH="../tmp/step2_keep.list"
...
do
    case $opt in
        "Step 1: Create a all.list")
            ...
            $SCRIPT_DIR/../step01-find/listAllReverse.sh $SCRIPT_DIR/$STEP1_MP3_PATH "$STEP1_FILE_FILTER" > $SCRIPT_DIR/$STEP1_TMP_ALL_FILES_PATH
            echo "    Done step 1" >&2;
            break # Exit the select loop
            ;;
        "Step 2: Create a keep.list")
            echo "    Execute step 2..." >&2;
            $SCRIPT_DIR/../step02-keep-or-left/keepList.sh $SCRIPT_DIR/$STEP1_TMP_ALL_FILES_PATH > $SCRIPT_DIR/$STEP2_TMP_FILES_KEEP_PATH
            echo "    Done step 2" >&2;
            break # Exit the select loop
            ;;
        "Step 3: Tag mp3")
            echo "    Execute step 3..." >&2;
            $SCRIPT_DIR/../step03-tag-track/tagList.sh $SCRIPT_DIR/$STEP2_TMP_FILES_KEEP_PATH
            echo "    Done step 3" >&2;
            break # Exit the select loop
            ;;
        "Quit")
