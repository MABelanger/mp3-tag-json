...
import { getHasBassFilter, getBassBound } from "./utils";
  ...
  useEffect(() => {
    ...
    async function performSearch() {
      ...
      try {
        ...
        const bassBound = getBassBound(filters.bass);
        ...
        if (queryRequirements.length > 0) {
          ...
          while (cursors.every((c) => c !== null)) {
            ...
            if (allMatch) {
              ...
              if (hasBassFilter) {
                ...
                if (
                  itemBassNum < bassBound.min ||
                  itemBassNum > bassBound.max
                ) {
                  keepsItem = false;
                }
              }
          ...
          if (hasBassFilter) {
            // Native database range match: bassBound.min <= value <= bassBound.max
            const bassRange = IDBKeyRange.bound(bassBound.min, bassBound.max);
          ...
          request.onsuccess = (event) => {
            ...
            if (cursor) {
              ...
              if (!hasBassFilter && hasBassFilter) {
                ...
                const itemBassNum = Number(item.bass);
                if (itemBassNum < bassBound.min || itemBassNum > bassBound.max)
                  keepsItem = false;
              }
              if (!hasBassFilter && filters.bpm && item.bpm != filters.bpm) {
                keepsItem = false;
